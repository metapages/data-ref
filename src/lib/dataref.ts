...
import { DataRef, DataRefType, DataRefTypeDefault, InputsRefs, UploadFunc } from "./dataref-types";
...
export const copyLargeBlobsToRemote = async (inputs: InputsRefs | undefined, upload :UploadFunc): Promise<InputsRefs | undefined> => {
  if (!inputs) {
    return;
  }
...
export const base64ToDataRef = async (value: string, options: {upload:(value:string)=>Promise<DataRef<string>>, ignoreHash?: boolean, maxLengthUnmodified:number}): Promise<DataRef> => {
